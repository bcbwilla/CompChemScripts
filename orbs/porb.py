...
'''
    Produces a list of orbitals for use with dos script.
    
    Takes two command line arguments: a string containing elemtents and orbitals
    and the filename of the file to search. 

    : python porbs.py string filename.log

     The format for the data string is

    'Atomic Symbol1: orb1 orb2 orb3, atom#1 atom#2; Atomic Symbol2: orb1 orb2, atom#1; ... etc.'
    
    If you do not want to include atom numbers, simlpy leave them off, e.g.:
    
    'Atomic Symbol1: orb1 orb2 orb3; Atomic Symbol2: ...' 
     
    Note:  The atom number is the number of the atom in your system as designated by Gaussian,
    NOT the atomic number.

   
    Ben Williams, 10/24/2012
'''
...
def process_input(orbs):
    '''This method processes the command line input'''
    
    sorbs = orbs.split(";") #Split each element
    orb_dict = {}

    for entry in sorbs:
        atom = entry.split(':') #Split element name from orbtial data       
        if "," in atom[1]:      #If there is also atom number data
            ent = atom[1].strip().split(",")
            data = []
            for i in ent:
                data.append(i.split())
        else:
            data = atom[1].strip().split()
        orb_dict[atom[0].strip()] = data  #Build dictionary
    
    return orb_dict

in_dictionary = process_input(' '.join(sys.argv[2:]))
...
def pull_orbitals(orb_d,in_d):
	...
	{'Atom Sym':[[orb1,orb2,...],[atom#1,atom#2]]...}
        ...
        if atom in orb_d:					        #check if atom is in orbital_dictionary
            for atom_num in orb_d[atom]:			#loop through atom num of each desired atom in orbital_dictionary
                for e in orb_d[atom][atom_num]:		#given a desired atom of certain number, search look at orbitals
                    if isinstance(in_d.items()[0][1][0],list):        #check if in_dictionary includes atom #s
                        if e[0] in in_d[atom][0] and str(atom_num) in in_d[atom][1]:     #if orbital is wanted, add to orbital_number_list
